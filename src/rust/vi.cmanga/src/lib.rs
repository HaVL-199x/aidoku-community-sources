...
fn cache_manga_page(id: &str) {
	unsafe {
		...
		CACHED_MANGA = Some(
			Request::new(
				format!("https://cmanga1.com/{id}").as_str(),
				HttpMethod::Get,
			)
			.data(),
		);
...
fn get_manga_list(filters: Vec<Filter>, page: i32) -> Result<MangaPageResult> {
	let mut url = format!("https://cmanga1.com/search.php?act=timnangcao&page={page}");
	...
	for elem in elems {
		...
		let id = url.replace("https://cmanga1.com", "");
...
fn get_manga_details(id: String) -> Result<Manga> {
	...
	let url = format!("https://cmanga1.com{}", id);
	let categories = html
	...
	nsfw = if html
		.select("a[itemprop=contentRating][href='https://cmanga1.com/DoTuoi/18/']")
		.array()
		.len() > 0
	{
...
fn get_chapter_list(id: String) -> Result<Vec<Chapter>> {
	...
	for elem in elems {
		...
		let chapter_id = url.replace("https://cmanga1.com", "");
...
fn get_page_list(_manga_id: String, chapter_id: String) -> Result<Vec<Page>> {
	let url = format!("https://cmanga1.com{chapter_id}");
	let html = Request::new(&url, HttpMethod::Get).html()?;
...
fn handle_url(url: String) -> Result<DeepLink> {
	// https://cmanga1.com/bon-te-tu-chinh-la-tien-dao-29234/828715-chap-247
